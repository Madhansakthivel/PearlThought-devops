    ...
    return {
        buildAssociationResolver ({ contentTypeUID, attributeName }) {
            ...
            return async (parent, args = {}, context = {})=>{
                ...
                const isBuiltInQueryField = (fieldName)=>{
                ...
                const inheritedStatus = context.rootQueryArgs?.status && context.rootQueryArgs?._originField && isBuiltInQueryField(context.rootQueryArgs._originField) ? context.rootQueryArgs.status : null;
                const statusToApply = args.status || inheritedStatus;
                const defaultFilters = isTargetDraftAndPublishContentType && statusToApply ? {
                    where: {
                        publishedAt: statusToApply === 'published' ? {
                            $notNull: true
                        } : {
                            $null: true
                        }
